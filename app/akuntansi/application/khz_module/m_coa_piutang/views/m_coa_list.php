...
<div class="container">
    ...
    <script>
        let data;
        $.getJSON("<?php echo base_url().'m_coa_piutang/json';?>", function(result) {
            data = result;
            $(".loadSpinner").hide();
            console.log(data);
            let indexKategori = 0;
        ...
        function addCoaModal(indexKategori, indexKelompok, kelompok, event){
            ...
            $('#modal #addbutton').on('click',function(){
                ...
                $.getJSON("<?php echo base_url().'m_coa_piutang/get_coa_id/';?>"+data.idCoa, function(newCoa) {
                    if(newCoa.length == 0){
                        var jqxhr = $.post( "<?php echo base_url().'m_coa_piutang/add_coa/';?>", {   kelompok: kelompok, idCoa: data.idCoa, nama_coa: data.namaCoa, saldo_awal: data.saldoAwal, saldo_normal_special: data.saldoNormal, quantity: data.quantity })
                        .done(function(res) {
                            $('#modal').modal('toggle');
                            console.log(res);
                            ...
                            if(res > 0){
                                ...
                                $.getJSON("<?php echo base_url().'m_coa_piutang/get_coa/';?>"+res, function(newCoa) {
                                    newCoa = newCoa[0];
        ...
        function editCoaModal(indexKategori, indexKelompok, event, uniqid) {
            ...
            $.getJSON("<?php echo base_url().'m_coa_piutang/get_coa/';?>"+uniqid, function(result) {
                result = result[0];
                fillModal(result);
                ...
                console.log(data);
                var jqxhr = $.post( "<?php echo base_url().'m_coa_piutang/update_coa/';?>", {   uniqid: uniqid, id_kelompok_coa: result.id_kelompok_coa, nama_coa: data.namaCoa, saldo_awal: data.saldoAwal, saldo_normal_special: data.saldoNormal, quantity: data.quantity })
                .done(function(res) {
                    console.log(res);
                    if(res){
                        $('#modal').modal('hide');
        ...
        function editCoa(uniqid, event){
            ...
            var jqxhr = $.post( "<?php echo base_url().'m_coa_piutang/update_coa/';?>", {   uniqid: uniqid, nama_coa: data.namaCoa, saldo_awal: data.saldoAwal, saldo_normal_special: data.saldoNormal, quantity: data.quantity })
            .done(function(data) {
