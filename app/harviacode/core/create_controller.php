...
$string .= "\n\n    public function index()
    {
        ...
        \$this->template->load('template_admin','$c_url/$v_list', \$data);
    }";
...
$string .="\n\n    public function index()
    {
        \$this->template->load('template_admin','$c_url/$v_list');
    } 
...
$string .= "\n\t    );
            \$this->template->load('template_admin','$c_url/$v_read', \$data);
        } else {
            \$this->session->set_flashdata('message', 'Record Not Found');
            redirect(base_url('$c_url'));
        }
    }
    ...
    public function create() 
    {
        \$data = array(
            'button' => 'Create',
            'action' => base_url('$c_url/create_action'),";
foreach ($all as $row) {
    $string .= "\n\t    '" . $row['column_name'] . "' => set_value('" . $row['column_name'] . "'),";
}
$string .= "\n\t);
        \$this->template->load('template_admin','$c_url/$v_form', \$data);
    }
...
$string .= "\n\t    );
    ...
    public function update(\$id) 
    {
        ...
        if (\$row) {
            \$data = array(
                'button' => 'Update',
                'action' => base_url('$c_url/update_action'),";
foreach ($all as $row) {
...
if ($export_word == '1') {
    $string .= "\n\n    public function word()
    {
        ...
        \$this->template->load('template_admin','" . $c_url ."/". $v_doc . "',\$data);
    }";
}
...
if ($export_pdf == '1') {
    $string .= "\n\n    function pdf()
    {
        ...
        \$html = \$this->template->load('template_admin','" . $c_url ."/". $v_pdf . "', \$data, true);
